...
func (r *CategoryRepository) List(ctx context.Context) ([]domain.Category, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, section, icon, description, kind, is_system, allows_negative, is_fixed, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.Name, &c.Section, &c.Icon, &c.Description, &c.Kind, &c.IsSystem, &c.AllowsNegative, &c.IsFixed, &createdAt); err != nil {
...
func (r *CategoryRepository) GetByID(ctx context.Context, id int64) (domain.Category, error) {
	...
	err := r.db.QueryRowContext(ctx,
		...
		Scan(&c.ID, &c.Name, &c.Section, &c.Icon, &c.Description, &c.Kind, &c.IsSystem, &c.AllowsNegative, &c.IsFixed, &createdAt)
	switch {
